$("#scroll_id").click(() => {
  ...
  var angle = ($('.container-circle').data('angle') + 180) || 180;
  $('.container-circle').css({'transform': 'rotate(' + angle + 'deg)'});
  $('.container-circle').data('angle', angle);

  setTimeout(function() {
    var angle2 = ($('.container-circle').data('angle') + 180) || 180;

    $('.container-circle').css({'transform': 'rotate(' + angle2 + 'deg)'});
    $('.container-circle').data('angle', angle2);
  }, 200);
...
function myMassange(userGend) {
  let mass =
    ...
    let answer =
    '<div class="w2f_input" hidden name="answers">' + userGend + '</div>';
    $(".w2_right").append(answer);
...
function genderNext() {
  $(".chooseGenderM").click(() => {
    ...
    // let ccc =
    // '<div class="w2f_input">' + userGend + '</div>';
    // $(".w2_right").append(ccc);
...
function addArray() {
  ...
  end_massange13 = {
    m: "А ещё у меня для Вас отличные новости. Вы прошли онлайн-диагностику и тем самым стали участником розыгрыша этого препарата, который является нашим спонсором. Только сегодня, у Вас есть возможность получить «GiperoPlus» -  <span style='color: red;'>со скидкой или бесплатно</span>.",
  };
  end_massange14 = {
    m: "<span style='color: red;'>ИСПЫТАЙ УДАЧУ! НАЖМИ НА КОЛЕСО!.</span> <br> <br> Затем, чтобы получить «GiperoPlus», впишите своё имя и номер телефона в форму заказа ниже. Ваши данные отправляются напрямую производителю, больше никто не имеет к ним доступа. Вам перезвонит специалист и после уточнения всех необходимых деталей, в тот же день Вам будет отправлена посылка с курсом «GiperoPlus».",
  };
  massange.push(
    end_massange1,
    end_massange2,
...
function scrollTop() {
  var wrap = $(".content"),
    ...
    currentHeight = wrap.scrollTop(),
    wrapScrollHeight = wrap.prop("scrollHeight"),
    desiredHeight = wrapHeight - wrapScrollHeight;
  if (currentHeight > desiredHeight) {
    wrap.scrollTop(desiredHeight);
  }
}
...
function checkForm(){
  ...
  // проверка на ввод в поле с телефоном только цифр
  var phoneChoice = document.getElementById('userPhone').value;
  var templatePhone = /\d/;
  if (!templatePhone.test(phoneChoice)) {
    userPhone.setCustomValidity("В поле нужно ввести только цифры");
    return false;
  }

  // проверка на ввод поля с именем пользователя
  var nameChoice = document.getElementById('userName').value;
  var temlateName = /^[А-Яа-яЁё\s]/i;
  if (!temlateName.test(nameChoice)) {
    userName.setCustomValidity("Имя не корректно, должны быть только буквы!");
    return false;
  }
